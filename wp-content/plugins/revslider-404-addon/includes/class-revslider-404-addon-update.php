...
class RevAddOn404Update {
	...
	private $remote_url_info	= 'https://updates.themepunch.tools/addons/revslider-404-addon/revslider-404-addon.php';
	...
	protected function _check_updates() {
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			if(isset($data->basic)) {
				...
				//update_option('revslider_404_addon-stable-version', $data->full->stable);
	...
	public function _retrieve_update_info() {
		...
		$validated = get_option('revslider_404_addon-valid', 'false');
	...
	public function _retrieve_version_info($force_check = false) {
		...
		$last_check = get_option('revslider-404-addon-update-check-short');
		if($last_check == false){ //first time called
			$last_check = time();
			update_option('revslider-404-addon-update-check-short', $last_check);
		}
		...
		if(time() - $last_check > 172800 || $force_check == true){
			...
			update_option('revslider-404-addon-update-check-short', time());
			...
			$response = wp_remote_post($this->remote_url, array(
				'user-agent' => 'WordPress/'.$wp_version.'; '.get_bloginfo('url'),
				'body' => array(
					'item' => urlencode('revslider-gallery-addon'),
					'version' => urlencode(REV_ADDON_404_VERSION),
					'code' => urlencode($purchase)
				)
			));
			...
			if ( $response_code != 200 || is_wp_error( $version_info ) ) {
				update_option('revslider_404_addon-connection', false);
				return false;
			}else{
				update_option('revslider_404_addon-connection', true);
			}
			
			/*
			$version_info = json_decode($version_info);
			if(isset($version_info->version)){
				update_option('revslider_404_addon-latest-version', $version_info->version);
			}
			
			if(isset($version_info->notices)){
				update_option('revslider_404_addon-notices', $version_info->notices);
			}
			
			if(isset($version_info->dashboard)){
				update_option('revslider_404_addon-dashboard', $version_info->dashboard);
			}
			
			if(isset($version_info->deactivated) && $version_info->deactivated === true){
				if(get_option('revslider_404_addon-valid', 'false') == 'true'){
					//remove validation, add notice
					update_option('revslider_404_addon-valid', 'false');
					update_option('revslider_404_addon-deact-notice', true);
				}
			}
			*/
		}
		...
		if($force_check == true){ //force that the update will be directly searched
			update_option('revslider-404-addon-update-check', '');
		}
